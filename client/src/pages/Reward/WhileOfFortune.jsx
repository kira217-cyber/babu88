...
const WhileOfFortune = () => {
  ...
  const loadWheels = useCallback(async () => {
    ...
    try {
      ...
      const { data } = await api.get("/api/wheels", requestConfig);
  ...
  }, [token, requestConfig, selectedWheelId, isBangla]);

  const loadSelectedWheel = useCallback(async () => {
    if (!token || !selectedWheelId) {
      setWheel(null);
      return;
    }
  ...
  }, [token, selectedWheelId, requestConfig, isBangla]);
  ...
  const handleSpin = async () => {
    if (!isAuthenticated || !token) {
      toast.error(
        t("স্পিন করতে প্রথমে লগইন করুন", "Please log in before spinning"),
      );
    ...
    if (Number(userWallet.rewardCoin) < Number(wheel.spinCost || 0)) {
      toast.error(
        t("পর্যাপ্ত রিওয়ার্ড কয়েন নেই", "Insufficient Reward Coins"),
      );
    ...
    if (!eligibility?.eligible) {
      toast.error(
        eligibility?.message ||
          t(
            "আপনি এই হুইল স্পিনের শর্ত পূরণ করেননি",
  ...
  if (!isAuthenticated) {
    return (
      <div className="flex min-h-[70vh] items-center justify-center bg-[#570052] p-5">
        <div className="w-full max-w-md rounded-3xl border border-yellow-400/40 bg-black/50 p-8 text-center text-white">
          <FaTrophy className="mx-auto text-6xl text-[#ffc800]" />
          ...
          <p className="mt-3 text-sm text-white/65">
            {t(
              "হুইল স্পিন করতে প্রথমে লগইন করুন।",
          ...
          <button
            ...
            className="mt-6 w-full cursor-pointer rounded-full bg-[#ffc800] py-3 font-extrabold text-black"
          >
  ...
  return (
    <>
      <style>{`
        /*
          One complete sequence is 6 seconds:
          0s - 3s  : Reward coin glass effect
          3s - 6s  : Spin button glass effect
        */
        ...
        /* Spin effect starts only after the coin effect has finished. */
        .spin-glass-shine,
        .spin-bling {
          animation-delay: 3s;
        }
        ...
        <div className="relative z-10 mx-auto max-w-[1500px]">
          ...
          <div className="mb-5 flex flex-col items-center justify-center gap-4 lg:flex-row lg:justify-between">
            <div className="text-center lg:text-left">
              ...
              {/* <p
                className="mt-2 max-w-2xl text-xs font-semibold leading-5 drop-shadow-[0_2px_2px_rgba(0,0,0,.9)] sm:text-sm"
                style={{
                  color: design.descriptionColor,
                }}
              >
                {getText(wheel?.description)}
              </p> */}
            </div>
          ...
          <div className="grid grid-cols-1 gap-5 xl:grid-cols-[270px_minmax(0,1fr)_315px]">
            <PrizeList
              ...
              title={t("পুরস্কারের তালিকা", "Prize Pool")}
            />
                ...
                <>
                  ...
                  {!eligibility?.eligible && (
                    <div className="mt-7 w-full max-w-lg rounded-xl border border-red-300/50 bg-red-950/80 p-3 text-center text-xs text-red-100">
                      ...
                      {t(
                        "এই হুইল স্পিনের প্রয়োজনীয় শর্ত এখনো পূরণ হয়নি।",
    ...
    <div className="order-3 mt-5 overflow-hidden rounded-2xl border-2 border-yellow-400/80 bg-[#063b87]/95 shadow-2xl xl:mt-0">
      ...
      <div className="grid grid-cols-[1fr_auto] border-b border-white/10 px-4 py-2 text-[10px] font-bold text-white/50">
        <span>
          {activeTab === "winners"
            ? t("ব্যবহারকারী", "User")
            : t("তারিখ ও পুরস্কার", "Date & Prize")}
        </span>

        <span>{t("পুরস্কার", "Prize")}</span>
      </div>
                ...
                <div className="min-w-0">
                  <p className="truncate text-[11px] font-bold">
                    {activeTab === "winners"
                      ...
                      : getText(prize.text) || t("পুরস্কার", "Prize")}
                  </p>
        ...
        <div className="mt-5 rounded-2xl border border-yellow-300 bg-[#fff9dc] p-5">
          <p className="text-xs font-bold uppercase text-black/40">
            {wonPrize
              ...
              : t("এইবার কোনো পুরস্কার নেই", "No Prize This Time")}
          </p>
