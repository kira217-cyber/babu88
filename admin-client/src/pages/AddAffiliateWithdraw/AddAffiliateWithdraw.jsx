// src/pages/admin/AffiliateAddWithdraw.jsx
...
// ────────────────────────────────────────────────
// Shared styling classes (same as your AddWithdraw)
// ────────────────────────────────────────────────
const cardBase =
      ...
      <div className={`${cardBase} w-full max-w-md p-6`}>
        ...
        <p className="text-yellow-100/90 mb-6">
          ...
          <span className="font-semibold text-yellow-300">
            {methodName || "this affiliate withdraw method"}
          </span>
...
// ────────────────────────────────────────────────
// Main Component – AffiliateAddWithdraw
// ────────────────────────────────────────────────
const AffiliateAddWithdraw = () => {
  ...
  const {
    ...
    queryKey: ["affiliate-withdraw-methods"],
    queryFn: async () => {
  ...
  const onSave = async (values) => {
    ...
    try {
      ...
      if (selected?._id) {
        await api.put(
          `/api/affiliate-withdraw-methods/${selected._id}`,
          payload,
          {
            headers: { "Content-Type": "multipart/form-data" },
          },
        );
        toast.success("Affiliate method updated successfully");
      } else {
        await api.post(`/api/affiliate-withdraw-methods`, payload, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        toast.success("Affiliate method created successfully");
      }

      qc.invalidateQueries({ queryKey: ["affiliate-withdraw-methods"] });
      refetch();
  ...
  const confirmDelete = async () => {
    ...
    try {
      setSaving(true);
      await api.delete(`/api/affiliate-withdraw-methods/${id}`);
      toast.success("Affiliate method deleted successfully");
      qc.invalidateQueries({ queryKey: ["affiliate-withdraw-methods"] });
      refetch();
      if (selectedId === id) clearToCreate();
    } catch (e) {
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
            <div>
              <h2 className={headCls}>
                {isCreateMode
                  ? "Create New Affiliate Withdraw Method"
          ...
          <div className="mb-2">
            <div className="flex items-center justify-between mb-4">
              <h3 className={subheadCls}>
                Affiliate Withdraw Modal Input Fields
              </h3>
        ...
        <div className={`${cardBase} p-6 md:p-8`}>
          ...
          {isLoading ? (
            <div className="text-yellow-400/60 text-center py-16">
              Loading affiliate withdraw methods...
            </div>
          ) : list.length === 0 ? (
            <div className="text-yellow-400/50 text-center py-16">
              No affiliate withdraw methods found. Create your first one above.
            </div>
          ) : (
...
export default AffiliateAddWithdraw;
