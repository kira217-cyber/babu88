// src/pages/admin/Withdraw/AffiliateWithdrawRequest.jsx
...
const AffiliateWithdrawRequest = () => {
  ...
  const fetchData = async (page = meta.page) => {
    try {
      ...
      if (q) params.q = q; // ✅ optional (server ignore করলে সমস্যা নাই)
      if (status !== "all") params.status = status;

      const { data } = await api.get("/api/affiliate-withdraw-requests", {
        params,
      });
      ...
      toast.error(
        err?.response?.data?.message ||
          "Failed to load affiliate withdraw requests",
      );
    } finally {
      setLoading(false);
    }
  ...
  const approveNow = async () => {
    ...
    try {
      setActing(true);
      await api.patch(
        `/api/affiliate-withdraw-requests/${selected._id}/approve`,
        {
          adminNote: note,
        },
      );
  ...
  const rejectNow = async () => {
    ...
    try {
      setActing(true);
      await api.patch(
        `/api/affiliate-withdraw-requests/${selected._id}/reject`,
        {
          adminNote: note,
        },
      );
  ...
  return (
    <div className="w-full">
                ...
                <tbody className="bg-black/40">
                  {loading ? (
                    ...
                    list.map((r) => {
                      ...
                      // ✅ affiliate stored in "user" (your schema)
                          ...
                          <td className="px-4 py-4">
                            <div className="flex items-center gap-2">
                              <button
                                onClick={() =>
                                  navigate(
                                    `/affiliate-withdraw-request/${r._id}`,
                                  )
                                }
      ...
      <ConfirmModal
        ...
        description="Rejecting will refund the affiliate user's balance (as per your server rules)."
        confirmText="Reject"
...
export default AffiliateWithdrawRequest;
