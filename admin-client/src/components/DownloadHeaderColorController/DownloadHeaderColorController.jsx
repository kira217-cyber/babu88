...
const defaults = {
  name: "Default Download Header Theme",
  isActive: true,
  ...
  closeHoverBg: "#1a1a1a",
  closeHoverOpacity: 1,
  closeIconColor: "#e0e0e0",
  closeIconOpacity: 0.9,
  closeIconSize: 22,

  // Icon / logo box
  iconBoxBg: "#1a1a1a",
  fallbackLetter: "B",
  fallbackLetterColor: "#f5b400",
  ...
  btnText: "#000000",
  btnTextSize: 14,
  btnBorderColor: "#f5b400",
  btnBorderOpacity: 0.35,
  btnDisabledOpacity: 0.55,
};
...
const IntField = ({ label, value, onChange, min = 8, max = 48 }) => (
  <div className="flex flex-col sm:flex-row sm:items-center gap-3">
    <label className={`${labelCls} sm:min-w-[160px]`}>{label}</label>
    <input
      type="number"
...
const DownloadHeaderColorController = () => {
  ...
  const loadConfig = async () => {
    try {
      setLoading(true);
      const res = await api.get("/api/download-header-color");
      if (!res.data) {
        setDocId(null);
        setForm(defaults);
        return;
  ...
  const handleSave = async () => {
    try {
      ...
      const res = await api.put("/api/download-header-color", payload);
  ...
  const headerText = useMemo(() => {
    ...
    return docId
      ? "Download Header Color Controller (Edit)"
      : "Download Header Color Controller (Create New)";
  }, [docId, loading]);
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                <ColorField
                  ...
                  onChange={(v) => setVal("closeHoverBg", v)}
                />
                <NumField
                  label="Hover BG Opacity"
                  value={form.closeHoverOpacity}
                  onChange={(v) => setVal("closeHoverOpacity", v)}
                />
                <ColorField
                ...
                />
                <NumField
                  label="Close Icon Opacity"
                  value={form.closeIconOpacity}
                  onChange={(v) => setVal("closeIconOpacity", v)}
                />
                <IntField
                  label="Close Icon Size (px)"
                  value={form.closeIconSize}
                  onChange={(v) => setVal("closeIconSize", v)}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                <ColorField
                  ...
                  onChange={(v) => setVal("iconBoxBg", v)}
                />
                <div className="flex flex-col gap-1">
                  <label className={labelCls}>Fallback Letter</label>
                  <input
                    className={`${fieldCls} max-w-[140px]`}
                    value={form.fallbackLetter}
                    onChange={(e) => setVal("fallbackLetter", e.target.value)}
                    maxLength={1}
                  />
                </div>
                <ColorField
                  label="Fallback Letter Color"
                  value={form.fallbackLetterColor}
                  onChange={(v) => setVal("fallbackLetterColor", v)}
                />
                <IntField
                  label="Fallback Letter Size (px)"
                  value={form.fallbackLetterSize}
                  onChange={(v) => setVal("fallbackLetterSize", v)}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                <ColorField
                  ...
                  value={form.btnBg}
                  onChange={(v) => setVal("btnBg", v)}
                />
                <ColorField
                  label="Button Text Color"
                  value={form.btnText}
                  onChange={(v) => setVal("btnText", v)}
                />
                <IntField
                ...
                <ColorField
                  ...
                  value={form.btnBorderColor}
                  onChange={(v) => setVal("btnBorderColor", v)}
                />
                <NumField
                  label="Button Border Opacity"
                  value={form.btnBorderOpacity}
                  onChange={(v) => setVal("btnBorderOpacity", v)}
                />
                <NumField
                  label="Disabled Opacity"
                  value={form.btnDisabledOpacity}
                  onChange={(v) => setVal("btnDisabledOpacity", v)}
                />
              </div>
            </div>
          </div>
...
export default DownloadHeaderColorController;
