...
router.post("/", files, async (req, res) => {
  try {
    const { categoryId, providerId, providerName, status } = req.body;

    if (!categoryId) return res.status(400).json({ success: false, message: "categoryId required" });
    if (!providerId || !providerName) {
      return res.status(400).json({ success: false, message: "providerId & providerName required" });
    }
    ...
    // duplicate providerId in same category -> mongo error
...
router.put("/:id", files, async (req, res) => {
  try {
    ...
    if (!doc) return res.status(404).json({ success: false, message: "Provider not found" });
