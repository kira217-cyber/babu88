...
router.get("/affiliate-bridge/users", requireAuth, async (req, res) => {
  try {
    ...
    const [list, total] = await Promise.all([
      User.find(match)
        .select(
          "username phone commissionBalance gameLossCommissionBalance depositCommissionBalance referCommissionBalance gameWinCommissionBalance currency createdAt",
        )
        .sort({ createdAt: -1 })
        .skip(skip)
        .limit(limit)
 ...
 */
router.post("/affiliate-bridge/bridge/:userId", requireAuth, async (req, res) => {
  try {
    ...
    if (!user) {
      return res.status(404).json({ success: false, message: "Affiliate user not found" });
    }
    ...
    // ✅ Update by your exact formula
    user.commissionBalance = n(user.commissionBalance) + net;
...
router.post("/affiliate-bridge/bridge-all", requireAuth, async (req, res) => {
  try {
    ...
    const users = await User.find(match).select(
      "username commissionBalance gameLossCommissionBalance depositCommissionBalance referCommissionBalance gameWinCommissionBalance",
    );
