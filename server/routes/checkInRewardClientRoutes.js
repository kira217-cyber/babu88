...
const resetProgressForNewVersion = async ({ progress, setting }) => {
  ...
  progress.lastRewardType = "";
  progress.lastRewardAmount = 0;
...
router.get("/check-in-reward", requireAuth, async (req, res) => {
  try {
    ...
    /*
     * Admin setting update করলে version পরিবর্তন হবে।
...
router.post("/check-in-reward/claim", requireAuth, async (req, res) => {
  ...
  try {
    ...
    /*
     * Progress না থাকলে তৈরি করবে।
     * একই user-এর progress model-এ unique index থাকা উচিত।
     */
    try {
    ...
    /*
     * Admin setting update করলে নতুন version অনুযায়ী
     * Day 1 থেকে শুরু হবে।
     */
    ...
    const rewardAmount = Number(currentDay.amount || 0);

    if (!Number.isFinite(rewardAmount) || rewardAmount <= 0) {
      throw createError("Invalid Check-In reward amount");
    }

    if (!["balance", "reward_coin"].includes(currentDay.rewardType)) {
    ...
    previousProgress = {
      ...
      nextClaimAt: progress.nextClaimAt,
      completedCycles: progress.completedCycles || 0,
      lastRewardType: progress.lastRewardType || "",
      lastRewardAmount: progress.lastRewardAmount || 0,
    };

    /*
     * এই atomic update একই সময়ে দুইটি claim সফল হওয়া
    ...
    reservedProgress = await UserCheckInProgress.findOneAndUpdate(
      progressFilter,
      {
        $set: {
          ...
          lastClaimAt: now,
          nextClaimAt,
          lastRewardType: currentDay.rewardType,
    ...
    const creditField =
      currentDay.rewardType === "balance" ? "balance" : "rewardCoin";

    /*
     * কোনো TurnOver তৈরি হবে না।
     */
    const updatedUser = await User.findByIdAndUpdate(
      user._id,
      {
        $inc: {
          [creditField]: rewardAmount,
        },
      },
      {
        returnDocument: "after",
    ...
    return res.status(200).json({
      ...
      message:
        currentDay.rewardType === "balance"
          ? "Check-In balance reward claimed successfully"
          : "Check-In reward coin claimed successfully",

      claimedDay: {
        dayNumber: currentDay.dayNumber,
        dayName: currentDay.dayName,
        rewardType: currentDay.rewardType,
        amount: rewardAmount,
      },
    ...
    /*
     * Progress reserve হওয়ার পর credit ব্যর্থ হলে
     * আগের progress ফিরিয়ে দেওয়া হবে।
     */
    ...
    /*
     * Credit হওয়ার পর অপ্রত্যাশিত error হলে
     * credited amount reverse করার চেষ্টা করবে।
     */
    if (creditedUserId && creditedField && creditedAmount > 0) {
      try {
