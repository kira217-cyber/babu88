...
const getWheelEligibility = async ({ user, wheel, session = null }) => {
...
/* ======================================================
   CLIENT: GET ALL ACTIVE WHEELS
   GET /api/wheels
====================================================== */

router.get("/wheels", requireAuth, async (req, res) => {
  try {
    const now = new Date();
...
router.get("/wheels/my/history", requireAuth, async (req, res) => {
  try {
...
/* ======================================================
   CLIENT: WINNER LIST
   অবশ্যই /wheels/:id route-এর আগে রাখতে হবে
...
/* ======================================================
   CLIENT: GET SINGLE WHEEL
   GET /api/wheels/:id
====================================================== */

router.get("/wheels/:id", requireAuth, async (req, res) => {
  try {
    ...
    const eligibility = await getWheelEligibility({
      user,
      wheel,
    });
...
router.post("/wheels/:id/spin", requireAuth, async (req, res) => {
  ...
  try {
    ...
    /* =========================
         ...
         Unique spinId একই request
         দুইবার process হতে দেবে না।
      ========================= */
    ...
    /* ==================================================
         ...
         MongoDB transaction না থাকায় কোনো
         step fail করলে changes reverse হবে।
      ================================================== */

    let rollbackSuccessful = true;

    /*
     * Turnover তৈরি হলে delete।
     */
    if (turnover?._id) {
      try {
    ...
    /*
     * Wallet update হলে Balance এবং
     * Reward Coin reverse করা হবে।
     */
    if (walletUpdated) {
      try {
    ...
    /*
     * Rollback সফল হলে processing
     * history delete করে user-কে
     * পুনরায় Spin করার সুযোগ দেওয়া হবে।
     */
    if (spinHistory?._id) {
      try {
...
router.get("/admin/wheel-spin-history", async (req, res) => {
  try {
    const page = Math.max(Number.parseInt(req.query.page || "1", 10), 1);
